          ...
          <tr v-for="(item, index) in formData.items" :key="index">
            ...
            <td>
              <div class="input-with-dropdown">
                <input type="text" v-model="item.name" @click="toggleProductDropdown(index)"
                  placeholder="نام کالا را وارد کنید" class="product-input" />
                <div v-if="showProductDropdown && selectedProductIndex === index" class="dropdown-list">
                  <div v-if="products.length === 0" class="dropdown-item error-message">
                    هیچ محصولی در دیتابیس یافت نشد
                  </div>
                  <div v-else v-for="product in products" :key="product.id" @click="selectProduct(product, index)"
                    class="dropdown-item">
            ...
            <td>
              <div class="input-with-dropdown">
                <input type="text" v-model="item.buyer" @click="toggleProfileDropdown(index)"
                  placeholder="نام خریدار را وارد کنید" class="profile-input" />
                <div v-if="showProfileDropdown && selectedProfileIndex === index" class="dropdown-list">
                  <div v-if="profiles.length === 0" class="dropdown-item error-message">
                    هیچ مشتری در دیتابیس یافت نشد
                  </div>
                  <div v-else v-for="profile in profiles" :key="profile.id" @click="selectProfile(profile, index)"
                    class="dropdown-item">
